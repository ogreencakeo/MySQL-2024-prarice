...
-- usertbl이 제거가 되지 않는 이유는 뭘까요?
-- buytbl과 FK가 설정되어 있기 때문에 제거가 안되는 것이다.
...
-- alter table을 이용해서 default 제약조건 추가
alter table usertbl
...
-- 근데 먼저 buytbl에 외래키를 설정을 해놓고도 아래와 같이 데이터를 삽입하고
-- 싶다면, foreign key 기능을 off 시키는 방법도 있다.
-- off를 시키지 않으면 usertbl에 userid가 없는 데이터들은 삽입이 되지 않는다라는걸 확인했다.
-- 하여 아래와 같은 방법으로 삽입을 한 것이다.
-- 하지만, 아래와 같은 방법은 절대 권장하지 않는다.
-- 먼저 회원을 가입시키고 그 회원이 물건을 사게 만들어야지 아무나 물건을 사면 이상하다.
-- 절차가 맞지 않다. 하여 usertbl에 데이터를 먼저 삽입하고 buytbl에 데이터를
-- 넣는 것이 당연한 절차이다.
...
-- check 제약 조건
-- 원래 오라클이나 msSQL에는 check 제약조건이 존재한다.
-- 하지만, mySQL에서는 지원을 하지 않는다. 추후에 배울 trigger을 통해서
-- 해결할 수 있으니 별 문제가 없다.
alter table usertbl
	add constraint CK_birthYear
    check (birthYead >= 1900 and birthYead <= year(curdate()));
...
-- 두 테이블을 조인하여 아래와 같이 결과를 도출하면 분명 buytbl에는 12건의
-- 데이터가 있지만, 9건밖에 출력이 되지 않는다.
-- 왜냐면 KHD기 KKK로 바뀌었기 때문에 당연히 결과값이 나오지 않는다.
select U.userid, U.username, B.prodname, U.addr
	from usertbl U
    inner join buytbl B
    on U.userid = B.userid;
    
-- 외부조인을 해봐도 결과는 KHD가 없다.
-- KHD가 있었던 부분이 null로 나오는 것을 확인할 수가 있다.
select U.userid, U.username, B.prodname, U.addr
	from usertbl U
...
-- 근데, KHD가 KKK로 바꿔달라고 하면 외래키를 추가할 때 on update casecade구문을
...
alter table buytbl
	add constraint FK_usertbl_buytbl
    foreign key (userid) references usertbl(userid)
    -- 이 구문이 들어감으로써 usertbl의 PK가 변경되면서 
    -- 따라서 buytbl의 FK인 userid도 따라서 바뀐다.
    on update cascade;
