-- 아래 쿼리문은 항상 쌍으로 실행을 한다.
-- 설명을 하자면, 만약 sqldb가 존재한다면 삭제를 하고
-- 다시, sqldb를 만들어라는 의미를 지니고 있다.
...
create table buytbl(
	-- auto_increment 명령어
    -- mysql엔진이 데이터가 들어올때 마다, 1씩 자동증가 시켜줌
	num int auto_increment not null primary key, 
    -- userid는 여기서는 pk가 될 수 없다. 그 이유는 한 테이블에는 pk는 오로지
    -- 하나만 존재해야 한다.
    userId varchar(8) not null,
    prodName varchar(6) not null, 	-- 물품명
    groupName varchar(4), 			-- 분류
    price int, 						-- 단가
    ammount smallint not null, 		-- 수량
    -- usertbl에 있는 userid를 참조해라.
    -- 여기서는 usertbl이 외래키로써(fk)
...
(null, 'KBS', '운동화', null, 30, 2), -- fk는 중복 가능
